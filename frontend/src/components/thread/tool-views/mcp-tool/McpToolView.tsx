...
export function McpToolView({
  ...
  const fullToolName =
    result?.mcp_metadata?.full_tool_name || parsedTool.fullToolName;
  const argumentsCount =
    result?.mcp_metadata?.arguments_count ??
    Object.keys(parsedTool.arguments).length;
              ...
              <div className="flex items-start gap-2 p-3 rounded-md bg-indigo-50/70 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-600 dark:text-indigo-400">
                    This tool is provided by an external MCP server, enabling
                    dynamic integration with specialized services and data
                    sources.
                  </p>
                </div>
              </div>
